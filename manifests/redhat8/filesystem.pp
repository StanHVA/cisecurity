# cisecurity::redhat8::filesystem
#
# Implements Center of Internet Security filesystem controls.

class cisecurity::redhat8::filesystem (
  ...
  if $harden_system_file_perms == 'enabled' {
    ...
    if $cisecurity::redhat8::services::configure_at_allow == 'disabled' {
      file { '/etc/at.allow':
        ensure => file,
        group  => 'root',
        owner  => 'root',
    ...
    if $cisecurity::redhat8::services::configure_cron_allow  == 'disabled' {
      file { '/etc/cron.allow':
        ensure => file,
        group  => 'root',
        owner  => 'root',
